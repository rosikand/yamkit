...
<p>Measured positions remain encoder observations. The reference model uses the last committed 14D command as state; that cached policy state is distinct from the measured curves shown here. When <code>reference_contract.startup=configured_home_open_grippers_then_cached_start</code>, the first state is the completed home/open-grippers command. Earlier literal runs without that field seeded from measured state and preserved the previous gripper opening. New chunks store the actual 14D robot-unit input after client preprocessing in <code>trace.json → chunks[].policy_state</code>; historical recordings may lack it. Model input images are unchanged.</p>
