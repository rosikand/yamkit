...
MIN_WARM_SAMPLES = 50
REFERENCE_CONTRACT = {
    "id": "yam_upstream_literal_v1",
    "ref_revision": "9a46f908d5dfb8a999f9a2c236681bf2af74f4da",
    "planner": "floor_max14_delta_0p01_cap100_endpoint_linspace",
    "rate": "send_sleep_observe_then_1ms_for_multipoint",
...
def qualification_settings(profile, *, modal_app: str | None = None, call_mode: str = "remote",
    ...
    if controller_mode == "reference":
        result["reference_contract"] = dict(REFERENCE_CONTRACT)
    if backend == "external":
...
def _check_reference_evidence(settings, integrated, completed, reasons, counter):
    """Validate literal row consumption and direct dispatch under its exact build."""
    ...
    if (settings.get("call_mode") != "http" or settings.get("execution_mode") != "cuda_graph10"
            ...
            or not _same_value(settings.get("reference_contract"), REFERENCE_CONTRACT)
            or not _same_value(proof.get("reference_contract"), REFERENCE_CONTRACT)):
        reasons.append("Reference evidence lacks its literal planner, rate and direct-dispatch contract")
    ...
    try:
        phase_deadline = _number(proof.get("phase_deadline_monotonic_s"), "reference phase deadline")
    except QualificationError as exc:
        reasons.append(str(exc))
        phase_deadline = None
    expected_dispatches = expected_extra_sleeps = 0
    for index, event in enumerate(completed):
        ...
        deltas, counts = event.get("reference_row_max_deltas"), event.get("reference_row_counts")
        expected_counts = []
        if (not isinstance(deltas, list) or not isinstance(counts, list)
                or len(deltas) != chunk_steps or len(counts) != chunk_steps):
            reasons.append("Reference response lacks all literal row displacement/count measurements")
        else:
            for delta, count in zip(deltas, counts, strict=True):
                try:
                    distance = _number(delta, "reference maximum 14D displacement")
                    # Avoid overflow before the pinned 100-point cap. Counts 0
                    # and 1 both send the target once, as the upstream runner does.
                    expected = 100 if distance >= 1 else max(1, int(distance / 0.01))
                    expected_counts.append(expected)
                    if type(count) is not int or count != expected:
                        reasons.append("Reference row count differs from the literal floor/cap interpolation rule")
                except QualificationError as exc:
                    reasons.append(str(exc))
        points = sum(expected_counts)
        expected_dispatches += points
        expected_extra_sleeps += sum(count for count in expected_counts if count > 1)
        if counter(event.get("plan_dispatches"), "reference plan dispatches", minimum=chunk_steps) != points:
            reasons.append("Reference plan adds or removes literal interpolation points")
        try:
            age = _number(event.get("observation_age_at_return_s"), "reference observation age")
            observed = _number(event.get("observation_timestamp_monotonic_s"), "reference observation timestamp")
            ...
            admitted = _number(event.get("plan_admitted_monotonic_s"), "reference plan admission")
            deadline = _number(event.get("plan_deadline_monotonic_s"), "reference plan deadline")
            duration = _number(event.get("planned_duration_s"), "reference nominal plan duration")
            if (age > settings["max_observation_age_s"] or not observed <= started
                    or not started + prediction_s <= admitted < observed + settings["max_observation_age_s"]
                    or not admitted < deadline or deadline != phase_deadline
                    or not math.isclose(duration, points / settings["fps"], rel_tol=1e-9, abs_tol=1e-9)):
                reasons.append("Reference admission freshness, phase deadline or nominal point duration is invalid")
        except QualificationError as exc:
            reasons.append(str(exc))
    ...
    if counter(proof.get("multipoint_extra_sleep_calls"), "reference multi-point sleeps") != expected_extra_sleeps:
        reasons.append("Reference multi-point sleep count differs from the pinned execution loop")
    predictions = _rows(integrated.get("prediction_samples"), "reference prediction samples", reasons)
    if any(counter(event.get("actions_executed_during_prediction"), "reference inference overlap") != 0
           for event in predictions):
        reasons.append("Reference commands overlapped synchronous inference")
    # Observe actual fake SDK sends independently of engine counters. Native
    # pre-hardware warmup and every completed policy RPC must have zero sends.
    ...
    sdk_sends = integrated.get("sdk_sends_during_completed_rpc")
    if (not isinstance(sdk, list) or len(sdk) != len(returned)
            or not isinstance(sdk_sends, list) or len(sdk_sends) != len(returned)):
        reasons.append("Reference evidence lacks independent SDK overlap measurements")
        sdk = sdk_sends = []
    matched = 0
    for request, observed_pairs, observed_sends in zip(returned, sdk, sdk_sends, strict=False):
        if (counter(observed_pairs, "reference observed SDK overlap") != 0
